...
Syncs your accepted LeetCode submissions into `leetcode/topics/<topic>/<number>-<slug>/`, commits each solution, and pushes it. GitHub Actions runs it every 30 minutes and can repair an expired LeetCode session once.

## Setup

1. Install Node 22+, run `npm install`, then copy `.env.example` to `.env`.
...
3. Run locally with `node --env-file=.env sync.js`. Use `SYNC_GIT=false` to write files without committing.
4. Push this project to the repository that should contain the solutions. Add these Actions secrets: `LEETCODE_SESSION`, `LEETCODE_CSRF_TOKEN`, `LEETCODE_USERNAME`, `LEETCODE_PASSWORD`, and `GH_SECRETS_TOKEN`.

`GH_SECRETS_TOKEN` is a fine-grained PAT with **Actions secrets: write** access for this repository. `GITHUB_TOKEN` is supplied by GitHub Actions and pushes the solution commits. A local `refresh-session.mjs` run also needs `GITHUB_REPOSITORY=owner/repo`.
...
The sync uses the authenticated `submissionList` query for your recent submissions, filters `statusDisplay: Accepted`, then requests `submissionDetails(submissionId)` for source code and `question(titleSlug)` for `questionFrontendId`, `difficulty`, `content`, and `topicTags`. It stores the timestamp plus submission IDs at that timestamp in `leetcode/.sync-state.json`, so re-runs are no-ops and same-second submissions are not skipped.
